...
import com.wyu.service.ALLService;
...
//	}
	...
	public List<Employee> get_EmployeeList(Integer page,Integer size) {
		...
		for (int i = 0; i < size1; i++) {
			...
			Dept dept = deptDao.get_Info(data.getDeptId());
			data.setDept(dept);
			Job job = jobDao.get_Info(data.getJobId());
			data.setJob(job);
			list2.add(i, data);
		}
		return list2;
...
//用户管理
	...
	public User login(String username, String password) {
		// TODO Auto-generated method stub
		User user = userdao.get_login(username, password);
		return user;
	}
